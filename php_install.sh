...
# maybe not must
yum -y install lrzsz curl curl-devel libcurl-devel nslookup tcpdump man traceroute telnet wget sos-2.2-38.el6 openssh* zip unzip e2fsprogs e2fsprogs-devel ntp vim-enhanced python lsof iptraf strace dos2unix bind-utils
yum -y install bison libXpm-devel gc* make openssl openssl-devel pcre-devel zlib-devel ncurses-devel ncurses zutoconf libjpeg libjpeg-devel libpng libpng-devel freetype freetype-devel libxml2 libxml2-devel zlib zlib-devel glibc glibc-devel glib2 glib2-devel bzip2 bzip2-devel krb5-devel libidn libidn-devel libxslt-devel libevent libevent-devel libtool-ltdl libtool
